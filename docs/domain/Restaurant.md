...
- 최근 인기 음식점은 배치 실행 시각 기준 최근 7일, 30일, 90일 안에 생성된 삭제되지 않은 `CONFIRMED`, `NO_SHOW` 예약 건수 기준으로 계산한다.
- 인기 음식점 API는 실시간 예약 집계가 아니라 `restaurant_popularity` 집계 테이블을 기준으로 조회한다.
- `CANCELLED` 예약과 삭제된 예약은 인기 음식점 계산에서 제외한다.
- Restaurant 생성은 예약 건수가 0이므로 인기 음식점 캐시 무효화 대상이 아니다.
- Restaurant 수정/삭제, Reservation 생성/상태 변경/삭제, 인기 음식점 배치 완료는 인기 음식점 캐시 무효화 대상이다.
